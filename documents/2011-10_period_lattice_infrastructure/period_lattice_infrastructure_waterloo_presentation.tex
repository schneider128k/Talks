...
\medskip
with Felix Fontein (Univiversity of Z\"urich)
 
\medskip
preprint arXiv:1111.1348
...
\medskip
\item compute the whole divisor classs group and the ideal class group
...
\begin{slide}{Abstract infrastructures}
...
\bigskip
\item $X$ is some finite set and its elements label the distinguished points of the torus
...
\vspace{-0.2cm}
\begin{itemize}
\item there is a surjective group homomorphism
...
\begin{slide}{Approximate homomorphism $f$}
...
\medskip
\item let $S$ be a finite subset of $X\times\mathbb{N}$ related to $\mathrm{Rep}(\mathcal{I})$

\medskip
\item $\Rightarrow$ we can efficiently compute a function $f : \mathcal{V} \rightarrow S$ with

\[
	f(v)=f(v') \Leftrightarrow \frac{v-v'}{N} \approx \lambda \mbox{ for some $\lambda\in\Lambda$}
\]
...
\medskip
\item the elements of the preimage $f^{-1}(v)\subset\mathcal{V}$ have the special form
\[
	v' = v + N \lambda + \xi_\lambda \;\;\;\mbox{with}\;\;\; \xi_\lambda\in(-1,1)^n
\]
\end{itemize}
...
\bigskip
\item a uniform superposition in $\mathbb{C}^\mathcal{V}:={(\mathbb{C}^{qN})}^{\otimes n}$ over a set of the form
\begin{align*}
\mathcal{M}  & = \{ v' = v + N \lambda + \xi_\lambda \mid \lambda\in\Lambda \text{ and } \xi_\lambda\in(-1,1)^n \text{ s.t. } v'\in\mathcal{V} \}
\end{align*}

is called a pseudo-periodic state with period lattice $\Lambda$

\bigskip
\item the offset $v$ is random

\medskip
\item we can prepare such pseudo-periodic states by evaluating the approximate homomorphism $f$ 
\end{itemize}
\end{slide}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{slide}{Computing the period lattice}

\begin{itemize}
\item given access to pseudo-periodic states with unknown random offsets, the task is determine an approximate
basis $\lambda'_1,\ldots,\lambda'_n$ of $\Lambda$

\medskip
\item 
a basis $\lambda'_1,\ldots,\lambda'_n$ is called $\gamma$-approximate iff
...
\vspace{-0.25cm}
\begin{itemize}
\item let $\Lambda=R \mathbb{Z}$ be a 1d lattice with unknown period $R\in\mathbb{R}_{>0}$

\medskip
\item let $\mathcal{V}=\{0,\ldots,qN-1\}$

\bigskip
\item a uniform superposition in $\mathbb{C}^\mathcal{V}:={(\mathbb{C}^{qN})}$ over a set of the form
\begin{align*}
\mathcal{M}  & = \{ v' = v + m N R + \xi_m \mid m\in\Z \text{ and } \xi_\lambda\in(-1,1) \text{ s.t. } v'\in\mathcal{V} \}
\end{align*}

is called a pseudo-periodic state with period lattice $\Lambda$

\bigskip
\item the offset $v\in\{0,1,\ldots,\lfloor N R \rfloor\}$ is random
...
\begin{itemize}
\item assume $q > 2 R$ and $N > \tfrac{2}{R}$

\bigskip
\item prepare two pseudo-periodic states in $\mathbb{C}^{\mathcal{V}}$ with period $R$

\bigskip
\item embed each state into the larger space $\mathbb{C}^\mathcal{W}$ where $\mathcal{W}:=\{0,\ldots,2qN-1\}$

\bigskip
\item apply the QFT to each state

\bigskip
\item measure both states
...
\begin{itemize}
\item let $\Lambda^* = \tfrac{1}{R} \, \mathbb Z$ denote the dual lattice of $\Lambda= R \mathbb{Z}$

\medskip
\item for $\lambda^*\in\Lambda^*$, let 
\[
w_{\lambda^*} = 
[2q \lambda^*] \quad\quad \text{rounded to closest integer}
\]

\medskip
\item for all $\lambda^*\in\Lambda^*$ with $w_{\lambda^*} \in [0,2q\kappa N]$ for some fixed $\kappa\in(0,1)$, we have
\[
\Pr(w_{\lambda^*}) \ge \frac{1}{2qN} \cdot \frac{q}{R} \cdot \cos^2\big( \pi (\tfrac{1}{4} + \tfrac{1}{4qN} + 2\kappa) \big)
\]

\medskip
\item the number of such $\lambda^*$ is approximately $\tfrac{\kappa N}{\mathrm{det}(\Lambda^*)} = \kappa N R$
...
\medskip
\item for half of the pairs $(w_1,w_2)$, the corresponding $\lambda_i^*$ generate $\Lambda^*$

\medskip
\item $\Rightarrow$ the pair $(\tfrac{w_1}{2q},\tfrac{w_2}{2q})$ is an approximate generating set of $\Lambda^*$ with probability at least
\[
\frac{\kappa^2}{8} \cos^4\big( \pi (\tfrac{1}{4} + \tfrac{1}{4qN} + 2\kappa) \big) \ge 10^{-5} \quad \mbox{for } \kappa=\tfrac{1}{9}
\]
\end{itemize}
...
\item assume that $q$ and $N$ that are sufficiently large

\medskip
\item prepare $2n+1$ pseudo-periodic states in $\mathbb{C}^{\mathcal{V}}$ 

\medskip
\item embed each state into the larger space $\mathbb{C}^\mathcal{W}$ where $\mathcal{W}=\{0,\ldots,2nqN-1\}^n$

\medskip
\item apply the multidimensional QFT to each state

\medskip
\item measure each state
...
\vspace{-0.3cm}
\begin{itemize}
\item let $\Lambda^*$ denote the dual lattice of $\Lambda$

\medskip
\item for $\lambda^*=(\lambda^*_1,\ldots,\lambda^*_n)\in\Lambda^*$, let
\[
\mathcal{R}_{\lambda^*} = \big\{
(w_1,\ldots,w_n) \mid w_k \in \{ \lfloor 2nq \lambda_k^* \rfloor, \lceil 2nq \lambda_k^* \rceil \} 
\big\}
\]
...
\[
\Pr(\mathcal{R_{\lambda^*}} ) \ge 2^{n-1} \cdot \frac{1}{(2qnN)^n} \cdot \frac{q^n}{\mathrm{det}(\Lambda)} \cdot \cos^2\big( \pi (\tfrac{1}{4} + \tfrac{1}{4qN} + 2\kappa) \big) 
\]
...
\medskip
\item with probability at least $\tfrac{1}{4}$ they generate a full-rank sublattice $\Lambda^*_0$
...
\medskip
\item with probability at least $\frac{1}{4}-\hat{\zeta}>0.184$ all the $2n+1$ samples generate $\Lambda^*$ 
...
\[
	\frac{1}{4} \Big( \frac{1}{4}-\hat{\zeta} \Big) \left( \frac{c\kappa^n}{2 n^n} \right)^{2n+1}
\]

where $c=\cos^2\big( \pi ( \tfrac{1}{4} + \tfrac{1}{4qN} + 2\kappa) \big)$ and $\kappa = \tfrac{1}{9n}$
\end{itemize}
